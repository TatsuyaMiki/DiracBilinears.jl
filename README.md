...
To compute the spatial distribution of electron chirality $\tau^Z(\boldsymbol{r})$, use the following commands:
```Julia
using DiracBilinears
τz = calc_density(calc="$\mrm{\tau}$z", qedir="\$outdir/seedname.save")
```
...
The following command is used to calculate the matrix elements of electron chirality for $6\times 6 \times 6$ unit cells:
```Julia
using DiracBilinears
